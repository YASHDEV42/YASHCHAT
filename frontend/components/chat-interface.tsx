...
export default function ChatInterface({
  ...
  if (!currentUser._id) {
    return (
      <div className="flex items-center justify-center h-full text-muted-foreground">
        <p>Invalid current user information</p>
      </div>
    );
  }
  ...
  if (!chat.users.some((user) => user._id === otherUser._id)) {
    return (
      <div className="flex items-center justify-center h-full text-muted-foreground">
        <p>User not found in this chat</p>
      </div>
    );
  }
  if (!chat.users.some((user) => user._id === currentUser._id)) {
    return (
      <div className="flex items-center justify-center h-full text-muted-foreground">
        <p>Current user not found in this chat</p>
      </div>
    );
  }
  if (!chat._id || !chat.users || chat.users.length === 0) {
    return (
      <div className="flex items-center justify-center h-full text-muted-foreground">
        <p>Invalid chat information</p>
      </div>
